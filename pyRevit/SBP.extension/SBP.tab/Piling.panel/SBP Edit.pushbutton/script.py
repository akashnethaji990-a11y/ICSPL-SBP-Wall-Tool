...
def pick_piles():
    ...
    if pre:
        return pre
    try:
        refs = uidoc.Selection.PickObjects(ObjectType.Element, SR.PileFilter(),
                                           "Click any pile of the wall(s) to edit, then click Finish")
...
if not names:
    fail("Select piles of an SBP wall (marks like SBP1-H001).")
saved = SR.load_walls(doc)
missing = [n for n in names if n not in saved]
walls = [n for n in names if n in saved]
if not walls:
    fail("No saved settings for: {}.\nThis wall was made before SBP Edit existed. Run SBP Wall once on its line "
         "with the same wall name; after that SBP Edit works on it.".format(", ".join(missing)))
if missing:
    forms.alert("No saved settings for: {} (made before SBP Edit existed). Run SBP Wall once on its line with the "
...
for p in plans:
    ...
    try:
        ...
        if p["mode"] == "rebuild":
            ...
            p["copied"], p["lost"] = SR.restore_data(snap, piles_now)
        else:
            piles_now = p["cur"]
            if p["mode"] == "levels":
                p["check"] = SR.apply_levels(doc, piles_now, a["cutoff"], a["toe_hard"], a["toe_soft"])
        ...
        if p.get("elements") and all(e is not None for e in p["elements"]):
            if a["invisible"]:
                p["line_errors"] = SR.hide_lines(doc, p["elements"])
            else:
                p["line_errors"] = SR.show_lines(doc, p["elements"], p["data"].get("line_styles"))
        try:
...
for p in plans:
    ...
    if p.get("lost"):
        output.print_md("**Typed data NOT copied:** " + ", ".join(p["lost"]))
    for msg in p.get("line_errors", []):
        output.print_md("**Line style problem:** {}".format(msg))
if look:
    output.print_md("HARD/SOFT look (2D): {}".format(look))
